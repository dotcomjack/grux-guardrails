...
One project-specific point, because of what this library is. Reporting that Grux fails,
including bluntly and in public, is not a conduct problem and never will be. Six tags in
this repository are published as leaking credentials on purpose. Telling the maintainer
that the redactor missed something, or that a limitation is worse than the README admits,
is the whole point of the project and is welcome in any tone. What is not welcome is
aiming that at a person instead of at the code. **Security findings still go to the
private channel first**, per [SECURITY.md](SECURITY.md), and that is a disclosure rule
rather than a conduct rule.
...
the community leaders responsible for enforcement at **jack@dotcomjack.com**. All
